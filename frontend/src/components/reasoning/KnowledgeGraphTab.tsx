...
export default function KnowledgeGraphTab({ caseId }: { caseId: string }) {
  ...
  return (
    <div className="h-[700px] flex flex-col relative font-sans bg-[#020617] rounded-xl overflow-hidden border border-white/10 shadow-2xl">
      ...
      {/* ─── TOP BAR: FOCUS MODES ─── */}
      <div className="absolute top-4 left-1/2 -translate-x-1/2 flex items-center gap-2 p-1.5 bg-black/50 backdrop-blur-md border border-white/10 rounded-full shadow-2xl z-10">
      ...
      {/* ─── TOP LEFT: SEARCH ─── */}
      <div className="absolute top-4 left-4 flex flex-col gap-3 w-72 z-10">
        <div className="relative">
          ...
          <Input 
            ...
            className="pl-9 bg-black/60 backdrop-blur-md border-white/10 text-white shadow-xl h-9 text-sm"
          />
          {filteredSearchNodes.length > 0 && (
            <div className="absolute mt-1 w-full bg-[#0f172a] border border-white/10 rounded-lg shadow-2xl overflow-hidden">
              {filteredSearchNodes.map(n => (
                ...
                >
                  <span style={{ color: n.typeConfig.color }}>{n.typeConfig.icon}</span> {n.cleanLabel}
                </button>
              ))}
            </div>
          )}
      ...
      <AnimatePresence>
        ...
        {selectedNode && !selectedLink && (
          <motion.div 
            ...
            className="absolute right-0 top-0 h-full w-80 bg-[#020617]/90 backdrop-blur-2xl border-l border-white/10 shadow-2xl flex flex-col z-20"
          >
            <div className="p-5 border-b border-white/10 flex items-start justify-between">
               <div className="flex gap-4 items-center">
                 <div className="w-12 h-12 rounded-xl flex items-center justify-center text-2xl shadow-inner border border-white/5" style={{ backgroundColor: selectedNode.typeConfig.color + '20' }}>
                 ...
                 <div className="flex flex-col">
                   ...
                   <h2 className="text-lg font-bold text-white leading-tight mt-0.5 break-words max-w-[180px]">
                     {selectedNode.cleanLabel}
                   </h2>
                 </div>
               </div>
               <button onClick={() => { setSelectedNode(null); setHighlightNodes(new Set()); }} className="p-1.5 hover:bg-white/10 rounded-lg text-white/40">
                 <X className="w-5 h-5" />
               </button>
            </div>
        ...
        {selectedLink && (
          ...
          >
            <div className="p-5 border-b border-white/10 flex items-start justify-between">
               <div className="flex gap-4 items-center">
                 <div className="w-12 h-12 rounded-xl flex items-center justify-center text-2xl shadow-inner border border-white/5" style={{ backgroundColor: selectedLink.edgeConfig.color + '20' }}>
               ...
               <button onClick={() => { setSelectedLink(null); setHighlightNodes(new Set()); setHighlightLinks(new Set()); }} className="p-1.5 hover:bg-white/10 rounded-lg text-white/40">
                 <X className="w-5 h-5" />
               </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-6 scrollbar-thin">
               ...
               <div>
                 ...
                 <div className="flex items-center gap-3 p-3 bg-indigo-500/10 border border-indigo-500/20 rounded-lg text-indigo-300 text-sm font-medium">
                   <FileText className="w-4 h-4 shrink-0" />
                   <span className="truncate">{selectedLink.evidence_file?.replace(/^\d{14,}_/, "").replace(/_/g, " ") || "Multiple Sources"}</span>
                 </div>
               </div>
